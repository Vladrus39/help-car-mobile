...
const CACHE_NAME = 'helpcar-v1.0.71';
const urlsToCache = [
  '/',
  '/index.html'
...
self.addEventListener('install', (event) => {
  console.log('[SW] Installing Service Worker v1.0.71...');
...
self.addEventListener('activate', (event) => {
  console.log('[SW] Activating Service Worker v1.0.71...');
    ...
    Promise.all([
      ...
      self.clients.matchAll().then((clients) => {
        clients.forEach((client) => {
          try {
            client.postMessage({ type: 'SW_UPDATED', version: '1.0.71' });
          } catch (err) {
